...
public class Graph {
    ...
    Vertex v, u;
    String edgeKey = " "; // this is to make sure that there is no two repeated edges
    ...
    for (int i = 0; i < verticesNum - 1; i++) { // Make a connected graph
        ...
        edgeKey = u.getLabel()+"-"+v.getLabel(); // make edge key of the new edge
        addEdge(v, u, weight); // make an edge between them
         addedEdgeSet.add(edgeKey); // add the edge key to the set 
    }
    ...
    while (currentEdges < edgenum) { // While there are more edges to make
        ...
        if (index1 == index2) { // if two vertices are the same skip them 
            continue;
        }
            ...
            edgeKey = u.getLabel()+"-"+v.getLabel(); // make edge key of the new edge
        if (!addedEdgeSet.contains(edgeKey)) { // if the edge is not exist 
          
            weight = (int) ((Math.random() * 100) + 1);
            addEdge(v, u, weight); // make edge 
            currentEdges++;
            addedEdgeSet.add(edgeKey); // add the edge key to the set 
